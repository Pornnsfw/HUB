...
local function DoPlaceTower(TName, TPos)
    while true do
        ...
        -- Ensure the server response explicitly succeeds
...
function TDS:Place(TName, px, py, pz, ...)
    ...
    -- Force wait until placement packet succeeds
    ...
    return true -- Returns true strictly after placement finishes
end

function TDS:Upgrade(idx, PId)
    local t = self.PlacedTowers[idx]
    if not t then 
        -- If the tower index isn't ready yet, wait until it exists
        repeat
            task.wait(0.1)
            t = self.PlacedTowers[idx]
        until t
    end

    DoUpgradeTower(t, PId or 1)
    UpgradeHistory[idx] = (UpgradeHistory[idx] or 0) + 1
    return true -- Returns true strictly after upgrade finishes
end
